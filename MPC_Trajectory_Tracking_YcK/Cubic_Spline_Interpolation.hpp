...
private:
  Vec_f calc_s(Vec_f x, Vec_f y){ //  Calculate cumulative arc lengths represent the total distance traveled along a trajectory from the starting point to each specific point.
    Vec_f ds;
    Vec_f out_s{0};
    Vec_f dx = vec_diff(x); // used to calculate the differences between consecutive elements of vectors x and y, resulting in dx and dy.
    Vec_f dy = vec_diff(y);

    for(unsigned int i=0; i<dx.size(); i++){
      ds.push_back(std::sqrt(dx[i]*dx[i] + dy[i]*dy[i])); //  Euclidean distance between consecutive points in the trajectory defined by vectors x and y.
    }

    Vec_f cum_ds = cum_sum(ds); //  calculates the cumulative sum of the elements in the input vector.
